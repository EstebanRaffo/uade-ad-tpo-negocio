...
//		p.addProductoEnPedido(pr, 333);
		...
		/*
		 * try { Controlador.getInstancia().crearPedido("30-3992034-1"); } catch
		 * (ClienteException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); }
		 */
		
		try {
			List<PedidoView> result = new ArrayList<PedidoView>();
			result = Controlador.getInstancia().getPedidoByCliente(1);
			for(PedidoView p : result)
